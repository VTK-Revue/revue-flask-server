...
def get_group_members(group):
    return [User.query.get(group_participation.user) for group_participation in
            PersistentGroupParticipation.query.filter_by(persistent_group_id=group.id)]
...
def get_user_year_groups_by_year(user):
    ...
    for participation in YearGroupParticipation.query.filter_by(user_id=user.id):
        result[RevueYear.query.get(participation.year_id).year].add(YearGroup.query.get(participation.year_group))
    return result
